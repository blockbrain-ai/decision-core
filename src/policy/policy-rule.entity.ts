...
export function evaluateRule(rule: PolicyRule, context: PolicyContext): RuleEvaluation {
  ...
  // is about), NOT an authentication gate. A rule whose roles the caller does not
  // hold simply does not apply to that caller; it is the deny-unknown / default-deny
  // backstop (see wrapPdpDenyUnknown) that blocks an unidentified caller for whom no
  // ALLOW rule applies. Keep deny-unknown enabled for any public/untrusted surface so
  // a no-role caller cannot slip past a role-scoped rule into an implicit allow.
