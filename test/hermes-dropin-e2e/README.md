...
5. `post_tool_call` audit reaches the Decision Core server with the **real
...
Last verified: 2026-06-10 against Hermes 0.12.x. Result: 4 audit records,
both denies enforced, allowed tool dispatched. `PASS=true`.
...
# 2. Point HERMES_REPO in driver.py at your Hermes checkout, then set up a
...
# 4. Run the driver (Python venv with requests + pyyaml + the local hermes pkg)
python test/hermes-dropin-e2e/driver.py
# → prints E2E-RESULTS {...} and exits 0 on PASS
```

Files:
- `driver.py` — the e2e driver (edit `HERMES_REPO` / `DC_URL` / `TOKEN` at top)
